...
rated_data = "C:/Users/fedor/OneDrive/Documents/R/WCST_humans/_free_text_answers_agreed.xlsx"
...
Diffifulty of tasks was supposed to be a number 1-10, but it was also a free text answer, and some participants went outside the recommended range (e.g.: difficulty = 50). We capped these numbers to 10.

```{r Modify}

participantdata_inc$difficulty_modified <- ifelse(participantdata_inc$difficulty<1, 1, participantdata_inc$difficulty)
participantdata_inc$difficulty_modified <- ifelse(participantdata_inc$difficulty>10, 10, participantdata_inc$difficulty)
# 17: diff = 50; WNOL, solved it in 4 min, correct rule, 223, 708, 747
# 223: diff = 60; WNOLFS, did not solve it, said difficult and easy at the same time
# 708: diff= 65, WNOLFS, solved it in 2 min
# 747: diff= 50; WNOL, solved it in 8 min

# CHECK IF THEY SOLVED IT EASILY - MIGHT BE TYPO
...
conditions <- c("Letters In", "Letters Out", "No Letters", "Fixed Sequence", "Ambiguous Cards", "Moon Card", "Letters Only")
...
kable(partic_groups[,c(1, 2, 3, 6, 12, 11)], 
      digits = 2,
      col.names = c("Condition", "Number of participants", "Number of solvers", "Solution rate", "Median task time (min)", "Aha rate of solvers"))
```
...
* then, if the Fisher’s exact test was not significant, we compared task time by two-sample Wilcoxon test (same as the Mann-Whitney test) or by 
...
D = `r normality.D`, p = `r normality.p-value`
...
We analyzed the contingency table containing the number of solvers who reported Aha-feelings and number of solvers who did not report Aha-feelings in pairs of conditions with Fisher's exact tests. A p<0.05 means that the row/column association is statistically significant:

WLIN > WONLY: p = `r F3$p.value`   
WLOUT > WONLY: p = `r F4$p.value`   
WNOL > WONLY: p = `r F5$p.value`   

WLOUT > WLIN: p = `r F6$p.value`   
WNOL > WLOUT: p = `r F7$p.value`
...
WNOLFS-WNOLA: p = `r F8$p.value`   
MOONSQ-WNOLA: p = `r F9$p.value`
...
WNOLFS > WONLY: p = `r F10$p.value`   
WNOLA > WONLY: p = `r F11$p.value`   
MOONSQ > WONLY: p = `r F12$p.value`   

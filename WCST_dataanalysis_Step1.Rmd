...
datadir = "C:/Users/fedor/OneDrive/Documents/R/WCST_humans/RESULTS/Experiments/" # the directory where you put all the files from the server
...
samplesize = 78 # desired number of participants per condition
...
participantdata_f4 <- filter(participantdata_f3, wentback==FALSE)           # who went back to the instructions page after making some moves in the game or who refreshed the screen
...
participantdata_f8 <- filter(participantdata_f7, is.na(prolific_ID)==FALSE) # who did not have matching Prolific data (probably wrong manual ID)
participantdata_f9 <- filter(participantdata_f8, country_of_birth!="CONSENT REVOKED") # who revoked consent to use their data

f1 <- which(duplicated(participantdata$manual_ID)) # who played the game 2x (delete all attempts after the 1st)
f2 <- which(is.na(participantdata$manual_ID))     # who did not fill out the datasheet
f3 <- which(is.na(participantdata$resultfile))    # who did not start the game
f4 <- which(participantdata$wentback==TRUE) # who went back to the instructions page after making some moves in the game or who refreshed the screen
f5 <- which(is.na(participantdata$aha))           # who did not fill out survey
f6 <- which(participantdata$colorblindness==TRUE) # who checked color-blindness
f7 <- which(participantdata$task_time>15)    # who played longer than 15 min due to server error
f8 <- which(is.na(participantdata$prolific_ID)) # who did not have matching Prolific data (probably wrong manual ID)
f9 <- which(participantdata$country_of_birth=="CONSENT REVOKED")

excluded_indices <- unique(sort(c(f1,f2,f3,f4,f5,f6,f7,f8,f9)))
participantdata_allfilters <- slice(participantdata, -excluded_indices)

# Participants who gave up after starting the game
participantdata_gaveup <- participantdata_f3 %>%
  filter(status == "TIMED-OUT" | status == "RETURNED") %>%
  filter(is.na(aha)) # include those who timed out during the game, not after

# Exclusion reasons for Prolific feedback
actions <- rep.int(NA, sessions)

temp <- is.element(participantdata$index, participantdata_allfilters$index)
...
# Excluded participants on Prolific
participantdata_exc <- participantdata %>%
  filter(is.element(index, participantdata_allfilters$index) == FALSE) %>%
  arrange(prolific_ID)
...
# Exclude extra participants over desired sample size
WLIN <- filter(participantdata_allfilters, condition=="wlin")[1:samplesize,]
WLOUT <- filter(participantdata_allfilters, condition=="wlout")[1:samplesize,]
WNOL <- filter(participantdata_allfilters, condition=="wnol")[1:samplesize,]
WNOLFS <- filter(participantdata_allfilters, condition=="wnolfs")[1:samplesize,]
WNOLA <- filter(participantdata_allfilters, condition=="wnola")[1:samplesize,]
MOONSQ <- filter(participantdata_allfilters, condition=="moonsq")[1:samplesize,]
WONLY <- filter(participantdata_allfilters, condition=="wonly")[1:samplesize,]

participantdata_inc <- rbind(WLIN, WLOUT, WNOL, WNOLFS, WNOLA, MOONSQ, WONLY)

# Exclude participants from the other datasets too
...
* Who played longer than 15 minutes due to server error: `r nrow(participantdata_f7) - nrow(participantdata_f6)`
* Who did not have matching data on Prolific, probably due to wrong IDs: `r nrow(participantdata_f8) - nrow(participantdata_f7)`
* Revoked their consent to use their data: `r nrow(participantdata_f9) - nrow(participantdata_f8)`

All-in-all, we excluded `r nrow(participantdata) - nrow(participantdata_inc)` initialized experimental sessions and we were left with `r nrow(participantdata_inc)` participants.

We excluded extra participants if there were more than the desired sample size in a condition. In the end, we had `r samplesize` number of participants in each condition.
...
particgroups_gaveup <- participantdata_gaveup %>%
  group_by(condition) %>%
  summarize(
    Nbof_participants = n_distinct(index),
    Nbof_solvers = sum(solver, na.rm=T),
    Nbof_nonsolvers = Nbof_participants - Nbof_solvers,
    Failure_rate = Nbof_nonsolvers / Nbof_participants,
    Solution_rate = Nbof_solvers/Nbof_participants,
    Nbof_ahas = sum(aha, na.rm=T),
    Aha_rate = Nbof_ahas/Nbof_participants,
    Nbof_ahas_solvers = sum(solver*aha, na.rm=T),
    Aha_rate_solvers = Nbof_ahas_solvers / Nbof_solvers,
    Avg_task_time = mean(task_time, na.rm=T),
    Avg_nbof_moves = mean(nbof_moves, na.rm=T)
  )
...
# Save dataframes 
save(
  movedata_inc, participantdata_inc, participantdata_gaveup,
  file = paste(datadir, "_Step1_output.RData", sep="")
)
save.image()
